...
'*----------------------------------------------------------------------*
'* Modulo           : DPMPostgre - Estructura DB en PostgreSQL 14.1.3   *
...
'*------------|------|-------------------------------------|------|-----*
'* 19/02/2021 | 01-1 | Creacion del modulo DPMPostgre      | 2010 | 4.0 *
'*------------|------|-------------------------------------|------|-----*

Public Class DPMPostgre
	'Variables Publicas Modulo: DPMPostgre
	Public FHoy As Date
	...
	Private Sub DPMPostgre_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
		'*-------------------------------------------------------------------------------------------------*
		'* PMovilD - Pago Movil Dynamics                                              R1.V0.M0 (Prerelase) *
		'* JR Sistemas24, Co.                                                         Fecha: 12/02/2021    *
		'*-------------------------------------------------------------------------------------------------*
		'* Modulo           : DPMMariaDB - Revisar estructura de BD: PMovilD - MariaDB 10.3.1              *
		'* Rutina           : DPMPostgre_Load ---> Carga la forma DPMPostgre                               *
        ...
        Troot = "Estructura de la Base de Datos: PMovilD - (accdb-Access 2010/2019)"
        TableTree.Nodes.Add(Troot)

        For Each dr In PagoMDT.Rows
            If Not (dr("Table_Name")).ToString.StartsWith("MSys") Then
	...
	Private Sub TableTree_AfterSelect(ByVal sender As System.Object, ByVal e As System.Windows.Forms.TreeViewEventArgs) Handles TableTree.AfterSelect
		...
		'* JR Sistemas24, Co.                                                         Fecha: 12/02/2021    *
		'*-------------------------------------------------------------------------------------------------*
		'* Modulo           : DPMPostgre - Revisar estructura de BD: PMovilD - PostgreSQL                  *
        ...
        NameDB = TableTree.Nodes(0).Nodes(indice).Text
